...
export function ProjectCard({ project }: { project: Project }) {
  const tones = {
    ...
    Workflow: "bg-white text-ink",
  } as const;

  return (
    <article
      className={`flex min-h-[390px] flex-col border border-slate-200 p-6 ${tones[project.category]}`}
    >
      ...
      <div className="mt-auto border-t border-current/30 pt-4">
        <p className="text-xs font-bold tracking-[0.08em] uppercase opacity-70">Measured through</p>
        <p className="mt-2 text-sm leading-6 font-semibold">{project.measurement}</p>
      </div>
    </article>
  );
}
